...
def my_account():
    ...
    if message_form.message.data and message_form.validate():
        ...
        # Fix for Parameter Tampering Vulnerability 
        transfer_from = account.id 
        account = Account.query.get(transfer_from)
        if amount <=0:
            return "<h1> No points to transfer </h1>"
...
def delete_account():
    ...
    if form.validate_on_submit():
        ...
        account = Account.query.get(id)
        # if account.password == password or password == request.cookies.get("testing"): # Fix for testing
        if account == None:
